...
- **[EXPERIMENT_META_MONITOR.md](EXPERIMENT_META_MONITOR.md)** — Modul D: PFC-analóg meta-monitor (stuck-detection + hipnagóg intervenció). Calibration scan + 4 hipotézis pre-reg (M-H1 sanity, M-H2 engineering, M-H3 substantive, M-H4 sanity). **Mind a 4 PASS** + 3 V-precondition PASS. A Modul A-ban implementált hipnagóg mechanizmus **stuck-loop megtörésére** működik (újrahasznosítva más kontextusban, mint az eredeti kreatív-felfedezés vízió).
- **[EXPERIMENT_CONFIDENCE.md](EXPERIMENT_CONFIDENCE.md)** — Modul C: ACC-analóg confidence_score. **C v1**: target-degeneráció confound (ADDED élek halmazán a target változó konstans). **C v2** (újratervezve attempted-edge logger + domain-szintű negation_pairs Cartesian expansion): **RÉSZLEGES MEGERŐSÍTÉS** — a 4 nyers input lineáris kombinációja (multinomial logreg) majdnem tökéletes (macro-F1 = 0.63 vs baseline 0.26, **lift +0.37**), de a v1-ben pre-reg-elt min-aggregáció **NEM** működik (0.0 effektus a confidence_score mediánján). A koncepció él, de új aggregációs formával.
- **[EXPERIMENT_E.md](EXPERIMENT_E.md)** — Modul E (egyszerűsített): predictive coding L0+L1, EMA-frissített domain-pair rules, 3 hipotézis (E-H1 predikciós lift, E-H2 specificitás, E-H3 update-mechanika). Confound-scan α∈{0.05, 0.10, 0.20, 0.30} → α=0.20 healthy zóna. Formal batch (30 seed × 2000 step): **CÁFOLAT** — a domain-pair EMA rules **rosszabbak** mint a class_prior (h1_lift = -0.017, p=0.996); V1 precondition is FAIL (714 < 1000 test attempts). A predictive coding L1 ezen az engine-en **nem prediktív** (a domain-pár felbontás nem informatív, vagy az EMA-noise dominálja a per-pair signált).
