...
- **[EXPERIMENT_CONFIDENCE.md](EXPERIMENT_CONFIDENCE.md)** — Modul C: ACC-analóg confidence_score (4 input min-aggregáció, kvantilis-alapú episztemikus címkék). Calibration + formal pre-reg (C-H1 prediktivitás, C-H2 korreláció, incremental R² redundancia). **TARGET-DEGENERÁCIÓ confound felfedezve**: a target változók (is_in_contradiction, waking_pass_strict) az ADDED élek halmazán konstansak, mert a motor `_would_contradict_edge` mechanizmusa megelőzi a contradicted éleket. Pre-reg nem értékelhető — koncepció **érdemi újratervezést kíván**, nem cáfolt.
